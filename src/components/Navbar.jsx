...
function Navbar() {
  ...
  return (
    <div
      className={`fixed z-[999] w-full px-20 py-8 font-['Neue_Montreal'] flex justify-between items-center transition-transform duration-300 ease-in-out 
      ${isVisible ? 'translate-y-0' : '-translate-y-full'} 
      bg-[rgba(255,255,255,0.5)] backdrop-blur-lg border-b border-gray-200 shadow-lg`}
    >
      <div className='logo'>
        {/* SVG Logo */}
      ...
      {/* Links container */}
      <div className={`links sm:flex gap-10 items-center max-sm:flex-col max-sm:absolute max-sm:top-20 max-sm:right-0 max-sm:bg-black max-sm:w-full max-sm:transition-all max-sm:duration-500 max-sm:ease-in-out ${isDropdownOpen ? 'max-sm:flex' : 'max-sm:hidden'}`}>
        <Link
          to="services"
          smooth={true}
          duration={1000}
