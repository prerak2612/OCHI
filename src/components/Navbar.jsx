...
function Navbar() {
  ...
  return (
    <div
      className={`fixed z-[999] w-full px-20 py-8 font-['Neue_Montreal'] flex justify-between items-center transition-transform duration-300 ease-in-out ${isVisible ? 'translate-y-0' : '-translate-y-full'} max-sm:px-4`}
    >
      <div className='logo'>
      <svg className='max-sm:w-10 max-sm:h-10 max-sm:mr-35 ' width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className={`links flex-col sm:flex-row sm:flex gap-10 items-center max-sm:hidden`}>
        <Link
          to="services"
          smooth={true}
          duration={1000}
          className="cursor-pointer text-lg font-light transition-all duration-300 ease-in-out hover:text-gray-1000 hover:scale-105 hover:tracking-wide capitalize"
        >
          Services
        </Link>
        <Link
        ...
        <Link
          ...
          smooth={true}
          duration={1000}
          className="cursor-pointer text-lg font-light transition-all duration-300 ease-in-out hover:text-gray-1000 hover:scale-105 hover:tracking-wide capitalize ml-32"
        >
          Contact us
        </Link>
      </div>
      ...
      {isDropdownOpen && (
        <div className="sm:hidden absolute top-20 left-0 w-full bg-gray-800 p-5 flex flex-col items-center space-y-4 text-white">
          <Link
            to="services"
            smooth={true}
            duration={1000}
            className="text-lg capitalize hover:text-gray-1000"
          >
            Services
          </Link>
          <Link
